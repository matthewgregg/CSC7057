...
class TestIMU(IsolatedAsyncioTestCase):
    ...
    def test_constructor(self):
        ...
        i = imu.IMU(DEVICE_ADDRESS, GYRO_SCALE, ACC_REGISTERS, GYRO_REGISTERS, MAG_REGISTERS, ACC_BIAS, GYRO_BIAS,
                  MAG_BIAS, *setup)
        self.assertIsInstance(i.bearing, float)

        i2 = imu.IMU(DEVICE_ADDRESS[0], GYRO_SCALE, ACC_REGISTERS, GYRO_REGISTERS, None, None, None,
                  None, *setup1)
        self.assertIsInstance(i2.bearing, float)

    async def test_start_tasks(self):
        await asyncio.gather(self.i.stream_readings([0, 0, [self.time]]))
        self.time += timedelta(seconds=1)
        await asyncio.gather(*[self.i.stream_readings([0, 0, [self.time]]), self.get_bearing()])
        self.time += timedelta(seconds=1)
        await asyncio.gather(*[self.i.stream_readings([0, 0, [self.time]]), self.get_adjusted_elevation()])
        self.time += timedelta(seconds=1)
        await asyncio.gather(*[self.i.stream_readings([0, 0, [self.time]]), self.get_readings()])
